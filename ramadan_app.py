"""
╔══════════════════════════════════════════════════════════╗
║     RAMADAN APP — Day 1 + Day 2 + Day 3 + Day 4 + Day 5 ║
...
║  Day 5 : mergeCardProps()       →  React Prop Merger     ║
╚══════════════════════════════════════════════════════════╝
"""
...
# ══════════════════════════════════════════════════════════
#  DAY 5 — React Card Prop Merger (Python equivalent)
#  mergeCardProps(defaults, custom)
# ══════════════════════════════════════════════════════════

def mergeCardProps(defaults: dict, custom: dict = None) -> dict:
    """
    Merges default and custom card props. Custom values override defaults.
    Mirrors JavaScript's object spread: { ...defaults, ...custom }

    React / JS equivalent:
        function mergeCardProps(defaults, custom = {}) {
            return { ...defaults, ...custom };
        }

    Args:
        defaults (dict): Base props — the component's built-in defaults.
        custom   (dict): User-supplied overrides. Missing = no overrides.

    Returns:
        dict: A NEW merged dict — originals are never mutated.
              Custom keys WIN over default keys on collision.

    Key rules:
        - custom overrides defaults on matching keys
        - extra keys in custom are added to the result
        - extra keys in defaults not in custom are kept
        - this is a SHALLOW merge (nested dicts are replaced, not deep-merged)
        - original dicts are NEVER mutated
    """
    if custom is None:
        custom = {}
    return {**defaults, **custom}      # ← Python dict unpacking = JS spread
...
<head>
  ...
  <style>
    *, *::before, *::after {{
    ...
    /* ═══════════════════════════════════════
       DAY 5 — REACT CARD PROP MERGER WIDGET
    ════════════════════════════════════════ */
    .merger-section {{
      position: relative; z-index: 2;
      width: 100%; max-width: 700px;
      margin: 0 auto 5rem;
      padding: 0 1.5rem;
      animation: fadeSlideDown .8s 1.4s ease both;
    }}
    .merger-section__label {{
      text-align: center;
      font-size: .7rem; letter-spacing: .3em;
      text-transform: uppercase; color: var(--gold);
      margin-bottom: 1.5rem; opacity: .8;
    ...
    .merger-signature .kw  {{ color: #c678dd; }}
    .merger-signature .fn  {{ color: #61afef; }}
    .merger-signature .param {{ color: #e5c07b; }}
    .merger-signature .spread {{ color: #61dafb; }}
    .merger-signature .type  {{ color: #56b6c2; }}

    /* Two-column prop editors */
    .prop-editors {{
      display: grid;
      grid-template-columns: 1fr 1fr;
      gap: 1rem;
    ...
    @media (max-width: 520px) {{
      .prop-editors {{ grid-template-columns: 1fr; }}
    }}
    .prop-editor {{
    ...
    .prop-editor__title.defaults {{ color: rgba(245,240,232,.45); }}
    .prop-editor__title.custom   {{ color: #61dafb; }}
    .prop-editor__title .badge-pill {{
      font-size: .58rem; padding: .15rem .5rem;
      border-radius: 10px; letter-spacing: .05em;
    }}
    .prop-editor__title.defaults .badge-pill {{
      background: rgba(255,255,255,.06);
      color: rgba(245,240,232,.3);
    }}
    .prop-editor__title.custom .badge-pill {{
    ...
    .prop-row input.key-input {{
      max-width: 90px;
      color: #e5c07b;
    }}
    .prop-row input.val-input {{
      color: #98c379;
    }}
    ...
    @media (min-width: 521px) {{
      .prop-editors {{ position: relative; }}
    ...
    .res-brace  {{ color: rgba(245,240,232,.4); }}
    .res-key    {{ color: #e5c07b; }}
    .res-colon  {{ color: rgba(245,240,232,.25); }}
    .res-val    {{ color: #98c379; }}
    .res-origin-default {{ color: rgba(245,240,232,.3); font-size:.65rem; margin-left:.4rem; }}
    .res-origin-custom  {{ color: rgba(97,218,251,.5);  font-size:.65rem; margin-left:.4rem; }}
    .res-overridden     {{ color: rgba(97,218,251,.8);  }}
    ...
    .legend-dot.default {{ background: rgba(245,240,232,.3); }}
    .legend-dot.custom  {{ background: #61dafb; }}
    .legend-dot.override {{ background: #61dafb; }}
  </style>
</head>
<body>
  ...
  <header>
    <section class="hero" aria-label="Ramadan Hero Banner">
      ...
      <!-- ── Card Prop Merger Widget (Day 5) ── -->
      <section class="merger-section" id="card-merger" aria-label="React Card Prop Merger">
        <p class="merger-section__label">✦ React Card Prop Merger ✦</p>
        ...
        <div class="merger-panel">
          ...
          <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:.5rem;">
            <span style="font-size:.75rem; color:rgba(245,240,232,.4); letter-spacing:.05em;">
              Object Spread Merge Demo
            </span>
            <span class="react-badge">
              <span class="react-dot"></span>
              React Props
            </span>
          </div>

          <!-- JS function signature -->
          <div class="merger-signature" aria-label="JavaScript function signature">
            <span class="kw">function</span>
            <span class="fn"> mergeCardProps</span>(<span class="param">defaults</span>,
            <span class="param"> custom</span> = <span class="spread">{{}}</span>)
            {{ <span class="kw">return</span>
            {{ <span class="spread">...defaults, ...custom</span> }}; }}
          </div>
          ...
          <div class="prop-editors">
            ...
            <!-- Defaults column -->
            <div class="prop-editor" id="defaultsEditor">
              <div class="prop-editor__title defaults">
                defaults
                <span class="badge-pill">base props</span>
              </div>
              <!-- rows injected by JS -->
            </div>

            <!-- Custom column -->
            <div class="prop-editor" id="customEditor">
              <div class="prop-editor__title custom">
                custom
                <span class="badge-pill">overrides</span>
              </div>
              <!-- rows injected by JS -->
            </div>
          ...
          <!-- Mobile arrow -->
          <div class="merge-arrow" aria-hidden="true">⬇ mergeCardProps() ⬇</div>

          <!-- Live result -->
          <div class="merger-result-wrap">
            <span class="merger-result-label">→ mergeCardProps(defaults, custom) returns</span>
            <div class="merger-result" id="mergerResult" aria-live="polite">
              <!-- filled by JS -->
            </div>
          </div>

          <!-- Legend -->
          <div class="merger-legend">
            <div class="legend-item">
              <span class="legend-dot default"></span> from defaults
            </div>
            <div class="legend-item">
              <span class="legend-dot custom"></span> from custom (added)
            </div>
            <div class="legend-item">
              <span class="legend-dot override"></span> custom overrides default
            </div>
          </div>
  ...
  <!-- ════════════════════════════════════════════
       DAY 5 — mergeCardProps  (React / JavaScript)
  ═════════════════════════════════════════════ -->
  <script>
    // ─────────────────────────────────────────────
    //  mergeCardProps
    //  React pattern: merges default and custom props.
    //  Custom values ALWAYS override defaults.
    //
    //  @param {{Object}} defaults  - base component props
    //  @param {{Object}} custom    - user overrides (default: empty object)
    //  @returns {{Object}}         - new merged object, originals untouched
    //
    //  Core: {{ ...defaults, ...custom }}
    //  Later keys win → custom always beats defaults on collision
    // ─────────────────────────────────────────────
    function mergeCardProps(defaults = {{}}, custom = {{}}) {{
      return {{ ...defaults, ...custom }};        // ← the entire logic, one line
    }}

    // ── State: editable prop arrays ──
    let defaultProps = [
      {{ key: "theme",    val: "light"  }},
      {{ key: "size",     val: "md"     }},
      {{ key: "bordered", val: "true"   }},
      {{ key: "rounded",  val: "false"  }},
    ];
    let customProps = [
      {{ key: "theme",    val: "dark"   }},
      {{ key: "label",    val: "Click me" }},
    ];

    // ── Render a prop editor column ──
    function renderEditor(containerId, propsArr, colorClass) {{
      const container = document.getElementById(containerId);
      // Keep the title div, rebuild rows after it
      const titleDiv  = container.querySelector(`.prop-editor__title`);
      container.innerHTML = "";
      ...
      propsArr.forEach((prop, i) => {{
        ...
        row.innerHTML = `
          <input class="key-input" type="text"
                 ...
                 data-src="${{containerId}}" />
          <span class="colon">:</span>
          <input class="val-input" type="text"
                 placeholder="value"
                 value="${{prop.val}}"
                 data-index="${{i}}" data-field="val"
                 data-src="${{containerId}}" />
          <button class="del-btn"
                  data-index="${{i}}"
                  data-src="${{containerId}}"
                  aria-label="Remove prop">✕</button>
        `;
      ...
      addBtn.dataset.src = containerId;
      container.appendChild(addBtn);
    }}

    // ── Render result box ──
    function renderResult() {{
      // Build plain objects from state arrays
      const defObj = Object.fromEntries(defaultProps.map(p => [p.key, p.val]));
      const cusObj = Object.fromEntries(customProps.map(p  => [p.key, p.val]));

      const merged = mergeCardProps(defObj, cusObj);   // ← Day 5 function

      const lines = Object.entries(merged).map(([k, v]) => {{
        const inDefault = k in defObj;
        const inCustom  = k in cusObj;
        const overridden = inDefault && inCustom;
        const onlyCustom = !inDefault && inCustom;

        let keyHtml = `<span class="res-key">${{k}}</span>`;
        let valHtml = `<span class="res-val ${{overridden || onlyCustom ? 'res-overridden' : ''}}">"${{v}}"</span>`;
        let originHtml = "";
        if (overridden)  originHtml = `<span class="res-origin-custom">← custom overrides</span>`;
        else if (onlyCustom) originHtml = `<span class="res-origin-custom">← from custom</span>`;
        else             originHtml = `<span class="res-origin-default">← from defaults</span>`;

        return `  ${{keyHtml}}<span class="res-colon">: </span>${{valHtml}}${{originHtml}}`;
      }});

      document.getElementById("mergerResult").innerHTML =
        `<span class="res-brace">{{</span><br>` +
        lines.join(",<br>") +
        `<br><span class="res-brace">}}</span>`;
    }}

    // ── Full re-render ──
    function renderAll() {{
      renderEditor("defaultsEditor", defaultProps, "defaults");
      renderEditor("customEditor",   customProps,  "custom");
      renderResult();
      attachListeners();
    }}
    ...
    function attachListeners() {{
      ["defaultsEditor","customEditor"].forEach(id => {{
        const el = document.getElementById(id);

        // Input changes
        el.querySelectorAll("input").forEach(inp => {{
          inp.addEventListener("input", e => {{
            const {{ index, field, src }} = e.target.dataset;
            const arr = src === "defaultsEditor" ? defaultProps : customProps;
        ...
        el.querySelectorAll(".del-btn").forEach(btn => {{
          btn.addEventListener("click", e => {{
            ...
            if (src === "defaultsEditor") defaultProps.splice(+index, 1);
            else                          customProps.splice(+index, 1);
            renderAll();
          }});
        }});

        // Add-prop button
        const addBtn = el.querySelector(".add-prop-btn");
        if (addBtn) {{
          addBtn.addEventListener("click", e => {{
            const src = e.target.dataset.src;
            if (src === "defaultsEditor") defaultProps.push({{ key: "", val: "" }});
            else                          customProps.push({{ key: "", val: "" }});
            renderAll();
          }});
        }}
      }});
...
# ══════════════════════════════════════════════════════════
#  TESTS — mergeCardProps
# ══════════════════════════════════════════════════════════

def run_merge_tests():
    test_cases = [
        # (defaults,                   custom,                    expected,                          description)
        ({"theme":"light","size":"md"},{"theme":"dark"},          {"theme":"dark","size":"md"},       "Custom overrides one key"),
        ({"theme":"light","size":"md"},{"label":"Click"},         {"theme":"light","size":"md","label":"Click"}, "Custom adds new key"),
        ({"theme":"light","size":"md"},{},                        {"theme":"light","size":"md"},       "Custom is empty → defaults unchanged"),
        ({},                           {"theme":"dark"},          {"theme":"dark"},                   "Defaults empty → only custom"),
        ({},                           {},                        {},                                 "Both empty → empty dict"),
        ({"a":1,"b":2},                {"a":9,"b":8},            {"a":9,"b":8},                      "Custom overrides ALL keys"),
        ({"x":1},                      {"y":2,"z":3},            {"x":1,"y":2,"z":3},                "Custom adds multiple new keys"),
        ({"theme":"light","bordered":True},{"theme":"dark","rounded":True},
                                          {"theme":"dark","bordered":True,"rounded":True}, "Mixed override and add"),
    ]

    print("=" * 68)
    print("  mergeCardProps — Test Results")
    print("=" * 68)
    all_pass = True
    for defaults, custom, expected, desc in test_cases:
        result = mergeCardProps(defaults, custom)
        # Ensure originals not mutated
        orig_defaults_copy = {**defaults}
        orig_custom_copy   = {**custom}
        mutated = (defaults != orig_defaults_copy or custom != orig_custom_copy)
        passed  = (result == expected) and not mutated
        ...
        if mutated: print(f"         ⚠️  MUTATION detected!")
        print()
    print("=" * 68)
